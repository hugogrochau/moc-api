...
/**
 * Base class that represents a row from the 'moc.health_insurance' table.
 *
 *
 *
* @package    propel.generator.MocApi.Models.Base
*/
abstract class HealthInsurance implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\MocApi\\Models\\Map\\HealthInsuranceTableMap';
    ...
    /**
     * Initializes internal state of MocApi\Models\Base\HealthInsurance object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>HealthInsurance</code> instance.  If
     * <code>obj</code> is an instance of <code>HealthInsurance</code>, delegates to
     * <code>equals(HealthInsurance)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|HealthInsurance The current object, for fluid interface
     */
     ...
     * @return $this|\MocApi\Models\HealthInsurance The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[HealthInsuranceTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[HealthInsuranceTableMap::COL_NAME] = true;
        }
    ...
    public function setCreated($v)
    {
        ...
        if ($this->created !== null || $dt !== null) {
            if ($this->created === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->created->format("Y-m-d H:i:s")) {
                ...
                $this->modifiedColumns[HealthInsuranceTableMap::COL_CREATED] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : HealthInsuranceTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : HealthInsuranceTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : HealthInsuranceTableMap::translateFieldName('Created', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = HealthInsuranceTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\MocApi\\Models\\HealthInsurance'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildHealthInsuranceQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see HealthInsurance::setDeleted()
     * @see HealthInsurance::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildHealthInsuranceQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                HealthInsuranceTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('moc.health_insurance_id_seq')");
        ...
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_CREATED)) {
            $modifiedColumns[':p' . $index++]  = 'created';
        }

        $sql = sprintf(
            'INSERT INTO moc.health_insurance (%s) VALUES (%s)',
    ...
    {
        $pos = HealthInsuranceTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['HealthInsurance'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['HealthInsurance'][$this->hashCode()] = true;
        $keys = HealthInsuranceTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\MocApi\Models\HealthInsurance
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\MocApi\Models\HealthInsurance The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_ID)) {
            $criteria->add(HealthInsuranceTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_NAME)) {
            $criteria->add(HealthInsuranceTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(HealthInsuranceTableMap::COL_CREATED)) {
            $criteria->add(HealthInsuranceTableMap::COL_CREATED, $this->created);
        }
    ...
    {
        $criteria = ChildHealthInsuranceQuery::create();
        $criteria->add(HealthInsuranceTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MocApi\Models\HealthInsurance (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \MocApi\Models\HealthInsurance Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildHealthInsurance is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildHealthInsurance The current object (for fluent API support)
     */
    public function setHealthInsuranceCards(Collection $healthInsuranceCards, ConnectionInterface $con = null)
    {
        ...
        foreach ($healthInsuranceCardsToDelete as $healthInsuranceCardRemoved) {
            $healthInsuranceCardRemoved->setHealthInsurance(null);
        }
    ...
    protected function doAddHealthInsuranceCard(ChildHealthInsuranceCard $healthInsuranceCard)
    {
        ...
        $healthInsuranceCard->setHealthInsurance($this);
    }
    ...
    public function removeHealthInsuranceCard(ChildHealthInsuranceCard $healthInsuranceCard)
    {
        if ($this->getHealthInsuranceCards()->contains($healthInsuranceCard)) {
            ...
            $healthInsuranceCard->setHealthInsurance(null);
        }
     ...
     * Otherwise if this HealthInsurance is new, it will return
     * an empty collection; or if this HealthInsurance has previously
     * been saved, it will retrieve related HealthInsuranceCards from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in HealthInsurance.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildHealthInsuranceCard[] List of ChildHealthInsuranceCard objects
     */
    public function getHealthInsuranceCardsJoinPatient(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildHealthInsuranceCardQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(HealthInsuranceTableMap::DEFAULT_STRING_FORMAT);
    }
