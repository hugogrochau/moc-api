...
use MocApi\Models\Map\UserTableMap;
...
/**
 * Base class that represents a row from the 'moc.user' table.
 *
 *
 *
...
abstract class User implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\MocApi\\Models\\Map\\UserTableMap';
    ...
    /**
     * Initializes internal state of MocApi\Models\Base\User object.
     */
     ...
     * @return $this|\MocApi\Models\User The current object (for fluent API support)
     */
    public function setPersonId($v)
    {
        ...
        if ($this->person_id !== $v) {
            $this->person_id = $v;
            $this->modifiedColumns[UserTableMap::COL_PERSON_ID] = true;
        }
    ...
    public function setAppId($v)
    {
        ...
        if ($this->app_id !== $v) {
            $this->app_id = $v;
            $this->modifiedColumns[UserTableMap::COL_APP_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UserTableMap::translateFieldName('AppId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\MocApi\\Models\\User'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PERSON_ID)) {
        ...
        if ($this->isColumnModified(UserTableMap::COL_APP_ID)) {
            $modifiedColumns[':p' . $index++]  = 'app_id';
        }

        $sql = sprintf(
            'INSERT INTO moc.user (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\MocApi\Models\User
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\MocApi\Models\User The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_PERSON_ID)) {
            $criteria->add(UserTableMap::COL_PERSON_ID, $this->person_id);
        }
        if ($this->isColumnModified(UserTableMap::COL_EMAIL)) {
        ...
        if ($this->isColumnModified(UserTableMap::COL_APP_ID)) {
            $criteria->add(UserTableMap::COL_APP_ID, $this->app_id);
        }
    ...
    public function hashCode()
    {
        ...
        //relation user_person_id_fkey to table moc.person
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MocApi\Models\User (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \MocApi\Models\User Clone of current object.
     * @throws PropelException
     */
